...
def meanAndVar(x, mass):
    mean = 0
    for i in range(len(x)):
        mean = mean + x[i] * mass[i]
    var = 0
    for i in range(len(x)):
        var = var + (mean - x[i])**2
    return mean, var
...
ax.bar([l+0.2 for l in sorted(levels_SEG)], [levels_SEG[l] for l in sorted(levels_SEG)], width=0.4, align='center', label="Segmental Simulation")
ax.set_xlabel("RNA per Population Level")
ax.set_ylabel("Mass")
...
fig.tight_layout()
fig.savefig(f'Viral_SSA_vs_SEG_c{c}_t200_interval.png')
plt.show()
...
x2, mass2 = orderDistribution(getDistribution("Segmental (c=" + str(c) + ",k=100).json", 200, 1))
print(meanAndVar(x2, mass2))
ax.bar([v+0.2 for v in x2], mass2, width=0.4, align='center', label="Segmental Simulation")
ax.set_xlim(left=-1, right=38)
ax.set_xlabel("RNA")
ax.set_ylabel("Mass")
ax.legend(shadow=True)
fig.tight_layout()
fig.savefig(f'Viral_SSA_vs_SEG_c{c}_t200.png')
plt.show()
