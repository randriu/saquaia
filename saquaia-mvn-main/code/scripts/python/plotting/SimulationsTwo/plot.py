...
##        'benchmark', 
##        '20230126_visuals', 
##        'TS_Visual_SEG+1GB_g0.0359_SSA_1.5', 
##        'plotting_data',
##        'sim82.png.csv'
##    )
##)
...
file2 = os.path.abspath(
    os.path.join(
        ...
        'benchmark', 
        '20230208_visual_TR', 
        'TR_Visual_SEG+1GB_g0.0359_HYB_1.5', 
        '0', 
        'plotting_data',
        'sim95.png.csv'
    )
...
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(20*cm, 4*cm))
...
for i in range(len(y_axis_labels)):
    ...
    ax2Twin.spines["right"].set_position(("axes", 1.0 + i * axis_distance))
    ax1Twin.get_yaxis().set_visible(False)

    axs[0].append(ax1Twin)
    axs[1].append(ax2Twin)
...
for label in data1:
    ...
    if label not in axis_id_for_label:
        continue
    ax = axs[0][axis_id_for_label[label]]
    ...
    ax.plot(data1[label]["x"], data1[label]["y"], color=color, linestyle=ls)

for label in data2:
    print(label)
    if label not in axis_id_for_label:
        continue
    ax = axs[1][axis_id_for_label[label]]
    color = color_for_label[label]
    ls = linestyle_for_label[label] if linestyle_for_label != None and label in linestyle_for_label and linestyle_for_label[label] != None else "-"
    renamed = label if not rename or label not in rename else rename[label]
    ax.plot(data2[label]["x"], data2[label]["y"], color=color, label=renamed, linestyle=ls)
...
for i in range(len(y_axis_labels)):
    ...
    if ylim_top and i in ylim_top and ylim_top[i] != None:
        ylt = ylim_top[i]
    axs[0][i].set_ylim(top=ylt, bottom=ylb)

    axs[1][i].set_ylabel(y_axis_labels[i])
    if color_for_axis and i in color_for_axis and color_for_axis[i] != None:
        axs[1][i].set_ylabel(y_axis_labels[i], color = color_for_axis[i])
        axs[1][i].tick_params(axis='y', labelcolor=color_for_axis[i])
...
fig.legend(bbox_to_anchor=(0.03, 1.0), shadow=True, loc="upper left", numpoints=2, ncol=ncol)
